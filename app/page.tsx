...
const nextStageItems = ["загрузка PDF/DOCX", "извлечение текста", "поиск по источникам", "генерация отчёта"];
  ...
  return (
    <svg className="plan-svg" viewBox="0 0 760 430" role="img" aria-label="Схема участка с объектом, парковкой, въездом и отступами">
      <defs>
        ...
        <filter id="softShadow" x="-20%" y="-20%" width="140%" height="140%"><feDropShadow dx="0" dy="12" stdDeviation="12" floodColor="#0f172a" floodOpacity="0.13" /></filter>
      </defs>
      ...
      <text x="247" y="197" textAnchor="middle" className="svg-building-title">ОБЪЕКТ</text><text x="247" y="221" textAnchor="middle" className="svg-building-note">18×10 м</text>
      <rect x="560" y="200" width="118" height="74" rx="16" fill="#e0f2fe" stroke="#60a5fa" strokeWidth="2" strokeDasharray="6 6" />
      <text x="619" y="234" textAnchor="middle" className="svg-parking-title">P</text><text x="619" y="255" textAnchor="middle" className="svg-parking-note">6 м/м</text>
      ...
      <circle cx="106" cy="198" r="8" fill="#86efac" /><circle cx="132" cy="222" r="7" fill="#86efac" />
      <text x="128" y="234" textAnchor="middle" className="svg-green">ЗЕЛЕНЬ</text>
      <text x="380" y="66" textAnchor="middle" className="svg-dimension">40 м</text><text x="84" y="96" className="svg-setback">отступ 3 м</text>
      <circle cx="698" cy="72" r="22" fill="#0b1f49" /><text x="698" y="79" textAnchor="middle" className="svg-north">N</text>
    </svg>
  );
}
...
export default function Home() {
  ...
  function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    setError("Доступ не подтверждён. Используйте демо-вход для просмотра кабинета.");
  }
    ...
    return (
      <main className="login-screen">
        <section className="login-card clean-login">
          <div className="login-brand"><span className="logo-mark">ГА</span><div><strong>ГрадоАналитик</strong><small>закрытый аналитический кабинет</small></div></div>
          <h1>Предпроектный анализ участка</h1>
          <p>Закрытый кабинет: участок, документы, предварительный вывод, риски, источники и схема размещения.</p>
          <form className="request-form login-form compact-login-form" onSubmit={handleLogin}>
            <button className="button primary demo-main-button" type="button" onClick={openDemoCabinet}>Войти в демо-кабинет</button>
            <details className="manual-login"><summary>Вход по выданному доступу</summary><label>Логин<input value={login} onChange={(event) => setLogin(event.target.value)} autoComplete="username" /></label><label>Пароль<input value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete="current-password" /></label><button type="submit">Войти</button></details>
            {error ? <p className="error-message">{error}</p> : null}
            <small className="form-note">Тестовый режим для демонстрации кабинета. Доступы согласовываются отдельно.</small>
          </form>
        </section>
      </main>
    );
  ...
  return (
    <main className="app-shell clean-shell">
      ...
      <section className="demo-strip"><div className="container demo-strip-inner"><strong>Демо-режим</strong><span>Показан интерфейс и логика кабинета. Реальная обработка документов подключается следующим backend-этапом.</span></div></section>
      {screen === "dashboard" ? (<section className="section clean-dashboard"><div className="container dashboard-layout"><div className="dashboard-main"><span className="eyebrow">Текущий анализ</span><h1>Краснодарский край, демонстрационный участок</h1><p>Предварительный статус: <strong>возможно при условиях</strong>. Для уверенного вывода нужно подтвердить ПЗЗ и сведения о ЗОУИТ.</p><div className="hero-actions"><button className="button primary" type="button" onClick={() => setScreen("plot")}>Продолжить анализ</button><button className="button secondary" type="button" onClick={() => setScreen("report")}>Открыть отчёт</button></div></div><aside className="summary-card dashboard-card action-card"><p className="card-label">Состояние</p><div className="status-list"><div className="status-row"><span>Участок</span><strong>{plotCreated ? "готов" : "демо"}</strong><small>основные данные заполнены примером</small></div><div className="status-row"><span>Документы</span><strong>{loadedDocs}/{docs.length}</strong><small>часть документов отсутствует</small></div><div className="status-row"><span>Отчёт</span><strong>{analysisStarted ? "сформирован" : "пример"}</strong><small>предварительный вывод без юридической силы</small></div></div><div className="next-stage-card"><strong>Следующий этап</strong><div>{nextStageItems.map((item) => <span key={item}>{item}</span>)}</div></div></aside></div></section>) : null}
