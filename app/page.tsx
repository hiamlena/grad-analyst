      ...
      <main className="login-screen">
        <section className="login-card clean-login">
          ...
          <form className="request-form login-form" onSubmit={handleLogin}>
            <label>Логин<input value={login} onChange={(event) => setLogin(event.target.value)} placeholder="demo" /></label>
            <label>Пароль<input value={password} onChange={(event) => setPassword(event.target.value)} placeholder="demo123" type="password" /></label>
            {error ? <p className="error-message">{error}</p> : null}
            <button type="submit">Войти</button>
            <button className="button secondary demo-button" type="button" onClick={openDemoCabinet}>Войти в демо-кабинет</button>
      ...
      <header className="site-header clean-header">
        <div className="container header-inner">
          ...
          <nav className="nav clean-nav" aria-label="Основная навигация">
            {navItems.map((item) => (<button className={navClass(item.key)} type="button" onClick={() => setScreen(item.key)} key={item.key}><span>{item.label}</span></button>))}
          </nav>
        ...
        <section className="section clean-dashboard">
          <div className="container dashboard-layout">
            <div className="dashboard-main">
              ...
              <p>Предварительный статус: <strong>возможно при условиях</strong>. Для уверенного вывода нужно подтвердить ПЗЗ и сведения о ЗОУИТ.</p>
              <div className="hero-actions"><button className="button primary" type="button" onClick={() => setScreen("plot")}>Продолжить анализ</button><button className="button secondary" type="button" onClick={() => setScreen("report")}>Открыть отчёт</button></div>
            </div>
            <aside className="summary-card dashboard-card action-card">
              ...
              <div className="next-stage-card"><strong>Следующий этап</strong><div>{nextStageItems.map((item) => <span key={item}>{item}</span>)}</div></div>
            </aside>
          </div>
        </section>
      ) : null}
